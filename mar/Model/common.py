...
def iwt_init(x, gpu_id_start=1):
    ...
    h = torch.zeros([out_batch, out_channel, out_height, out_width]).float().cuda(gpu_id_start)
...
class IWT(nn.Module):
    def __init__(self, gpu_id_start):
        super(IWT, self).__init__()
        self.requires_grad = False
        self.gpu_id_start = gpu_id_start
    def forward(self, x):
        return iwt_init(x, self.gpu_id_start)
...
class ShuffleBlock(nn.Module):
    def __init__(
        self, conv, in_channels, out_channels, kernel_size,
        bias=True, bn=False, gn=False, act=nn.ReLU(True), res_scale=1,conv_groups=1, num_groups_im=4):
...
class DWBlock(nn.Module):
    def __init__(
        ...
        bias=True, bn=False, gn=False, act=nn.ReLU(True), res_scale=1):
